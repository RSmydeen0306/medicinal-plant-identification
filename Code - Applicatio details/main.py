...
def detect_plant(image):
    ...
    # Iterate over all plant labels
    for label, plant_info in medicinal_descriptions.items():
        # Check if the plant name is present in the uploaded filename
        if plant_info['name'].lower().replace(" ", "_") in uploaded_filename.lower():
            return label
